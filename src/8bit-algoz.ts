...
export function parseOrderString(message: string): Partial<Order> | null {
    return parseOrderString01(message) ?? parseSpotOrder(message);
}

export function parseOrderString01(message: string): Partial<Order> | null {
    const patterns = [
        /.*#(?<coin>\S+).*Leverage: (?<leverage>[\d.]+)x.*Entry Zone: (?<entry>[\d.-]+)\s*Take Profit Targets:\s*(?<takeProfits>.+)\s*Stop Targets:\s*(?<sl>[\d.\d+\-% \)]+)/gusi,
        /.*#(?<coin>\S+).*Trade Type: (?<direction>long|short).*Leverage: (?:cross )?\((?<leverage>[\d.]+)x\).*Entry Zone: (?<entry>[\d.-]+)\s*Take Profit Targets:\s*(?<takeProfits>.+)\s*Stop Targets:\s*(?<sl>[\d.\d+\-% \)]+)/gusi,
        /.*#(?<coin>\S+).*Leverage: Cross \((?<leverage>[\d.]+)x\).*Entry Zone: (?<entry>[\d.-]+)\s*Take Profit Targets:\s*(?<takeProfits>.+)\s*Stop Targets:\s*(?<sl>[\d.\d+\-% \)]+)/gusi,
    ];
    ...
    for (const pattern of patterns) {
        ...
        const stopLoss = getSlValue(match.groups?.sl ?? '');
...
export function parseSpotOrder(message: string): Partial<Order> | null {
    if (!message.match(/spot/i)) {
        return null;
    }

    const patterns = [
        /.*#(?<coin>\S+).*Trade Type: (?<direction>spot).*Entry Zone: (?<entry>[\d.\- ]+)\s*Take Profit Targets:\s*(?<takeProfits>.+)\s*Stop Targets:\s*(?<sl>[\d.\d+\-% \)]+)/gusi,
    ];
